...
// Автор первоначального кода - Krid
// Код был немного изменен и принял текущий вид
...
begin
  Result := 0;
  if not (ImageType in [IMAGE_BITMAP, IMAGE_ICON]) then
    Exit;
  ...
  Result := SendMessageW(hBtn, BCM_FIRST + BCM_SETIMAGELIST, 0, lParam(@bi));
  if (Result = 0) then
    begin
      ImageList_Destroy(bi.ImgLst);
      Result := SendMessageW(hBtn, BM_SETIMAGE, ImageType, Image)
    end;
end;
